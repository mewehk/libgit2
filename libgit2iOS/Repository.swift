...
final public class Repository {
		...
		return remoteLookup(named: remoteSwift.name) { result in
			result.flatMap { remote in
				...
				if let credentials = credentials {
					...
					connect_result = git_remote_connect(remote, GIT_DIRECTION_PUSH, &callbacks, nil, nil)
				} else {
					connect_result = git_remote_connect(remote, GIT_DIRECTION_PUSH, nil, nil, nil)
				}
	...
	public func createLocalBranchForRemote(remoteBranch: Branch) -> Branch? {
		var oid = remoteBranch.oid.oid
		
		let mutable_ptr_oid = withUnsafeMutablePointer(to: &oid) {
			$0.withMemoryRebound(to: git_oid.self, capacity: 1, {
				return $0
			})
		}
		
		let ptr_oid = UnsafePointer(mutable_ptr_oid)
		var commit: OpaquePointer? = nil
		git_commit_lookup(&commit, self.pointer, ptr_oid)
		var branch: OpaquePointer? = nil
		
		let name = (remoteBranch.name as NSString).lastPathComponent
		git_branch_create(&branch, self.pointer, name, commit, 0)
		let upsreamResult = git_branch_set_upstream(branch, remoteBranch.name)
		guard upsreamResult == GIT_OK.rawValue else {
			return nil
		}
		guard let localBranch = branch else {
			return nil
		}
